...
export default class SlideEditorV extends Vue {
    ...
    // After saving, we are redefining `configs` as well as `slides`. So, the reference is lost.
    // Previously, within `onSaveFinish`, we called `loadConfig()`. This updated `slides`, which the editor component took
    // as a prop from the editor component. So this caused the editor component to get updated
    // Now this data in
    onSlideChange(): void {
        console.log(' ');
