            ...
            >
                <div class="flex flex-row justify-between items-center px-3 py-1">
                    ...
                    <div class="space-x-2 flex flex-row items-center">
                        ...
                        <div
                            v-if="editorStore.editorUnsavedChanges"
                        ...
                        <button
                            :disabled="!editorStore.editorUnsavedChanges"
                        ...
                        <button
                            ...
                            :disabled="!editorStore.editorUnsavedChanges || editorStore.editorSaving"
                            truncate-trigger
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                            ...
                            <span
                                ...
                                >{{
                                    editorStore.editorSaving ? $t('editor.savingChanges') : $t('editor.saveChanges')
                                }}</span
                            >
                    ...
                    <div class="flex flex-1 flex-col gap-0.5 md:flex-row justify-between">
                        ...
                        <div class="flex flex-col">
                            <span
                                ...
                                :class="{ italic: !productStore.metadata.title }"
                                v-truncate="{
                                    options: {
                                        delay: '200',
                                        placement: 'bottom-start',
                                        content: productStore.metadata.title,
                                        animateFill: true,
                                        touch: ['hold', 500]
                                    }
                                }"
                                >{{ productStore.metadata.title || $t('editor.untitledProject') }}</span
                            >
                            <span
                                tabindex="0"
                                class="line-clamp-1"
                                :class="productStore.metadata.title ? 'text-xs' : ''"
                                v-truncate="{
                                    options: {
                                        delay: '200',
                                        placement: 'bottom-start',
        ...
        <div class="editor-body flex">
            ...
            <div class="w-full">
                ...
                <slide-editor
                    ...
                    ref="slide"
                    :otherLangSlide="
                        productStore.slides[slideIndex]?.[
                            productStore.slides.find((slide) => slide.fr === editorStore.currentSlide) ? 'en' : 'fr'
                        ]
                    "
                    :lang="productStore.slides.find((slide) => slide.fr === editorStore.currentSlide) ? 'fr' : 'en'"
                    :slideIndex="slideIndex"
                    :isLast="slideIndex === productStore.slides.length - 1"
                    :uid="uuid"
                    @scroll-to-element="scrollToElement"
                    @slide-change="selectSlide"
...
import MetadataModalV from './metadata/metadata-modal.vue';
...
export default class EditorV extends Vue {
    ...
    onReconciliationRequest() {
        console.log(' ');
        console.log('onReconciliationRequest()s');
    ...
    created(): void {
        ...
        // TODO: what should we do if a new product is loaded into editor main, no changes are saved, and the user
        // then reloads? do we allow the product to error out into the home page? or should we allow the user to continue
        // maybe we autosave with a blank product?

        this.uuid = this.$route.params.uid as string;

        window.addEventListener('beforeunload', this.beforeWindowUnload);

        // If a reload has occurred, we must fetch the product from the server again
        if (!this.productStore.configLoaded) {
            this.productStore
        ...
        console.log(this.editorStore.currentSlide);
    ...
    mounted(): void {
        ...
        //TODO: fetch data from server and repopulate productStore if a reload has occured (rather than a route)
    ...
    restoreProperties() {
        console.log(' ');
        console.log('restoreProperties()');
        // TODO: When calling refreshConfig, `currentSlide` loses its reference to the `slides` in the `productStore`. So
    ...
    beforeRouteUpdate(to: RouteLocationNormalized, from: RouteLocationNormalized, next: () => void): void {
        console.log(' ');
        console.log('editor - beforeRouteUpdate()');
        // this.productStore.uuid = to.params.uid as string;
    ...
    beforeRouteLeave(to: RouteLocationNormalized, from: RouteLocationNormalized, next: (cont?: boolean) => void): void {
        console.log(' ');
        console.log('editor - beforeRouteLeave()');
    ...
    selectSlide(index: number, lang?: SupportedLanguages): void {
        ...
        setTimeout(() => {
            if (index === -1 || !this.loadSlides) {
                this.editorStore.currentSlide = '';
                console.log('current slide empty');
            } else {
     ...
     */
    // TODO: Now the custom-editor directly accesses the currentSlide. However the title within the respective
    // slide-toc-button doesn't update without this function (via the title-edit even). Need a better workaround
    // The slide-toc component loops over productStore.slides, and creates a component for each slide, within which
    // is a slide-toc-button for each lang config. But if currentSlide is a reference/pointer to a slide in
    // productStore.currentSlide, why doesn't it update automatically? The slide-toc component uses the draggable
    // component to create the list of slide toc elements. Perhaps this creates a copy of productStore.slides? Or
    // each slide of productStore.slides is copied?
    updateCustomSlide(slideConfig: Slide, save?: boolean, lang?: string): void {
        console.log(' ');
        console.log('updateCustomSlide()');
        console.log('slideConfig');
        console.log(slideConfig);
        const configLang = this.editorStore.configLang;

        this.editorStore.currentSlide = slideConfig;
        this.productStore.slides[this.slideIndex][(lang ?? configLang) as keyof MultiLanguageSlide] = slideConfig;
