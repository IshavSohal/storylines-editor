            ...
            >
                ...
                <router-link
                    ...
                    :to="{
                        ...
                        params: { lang: currLang === 'en' ? 'fr' : 'en', uid: productStore.uuid }
                    }"
                >
            ...
            <section class="px-7 md:px-20 py-5">
                ...
                <section v-if="!editExisting || editorStore.loadStatus === 'loaded'">
                    ...
                    <section>
                        ...
                        <a class="sub-link" @click="editorStore.swapLang()" tabindex="0">
                            {{
...
export default class MetadataEditorV extends Vue {
    ...
    created(): void {
        ...
        this.productStore.clearData(); // ensure that all product and editor data is cleared beforehand, just in case
        ...
        console.log('uuid in route');
        console.log(this.$route.params.uid as string);
    ...
    preview(): void {
        ...
        setTimeout(() => {
            const routeData = this.$router.resolve({
                name: 'preview',
                params: { lang: this.productStore.configLang, uid: this.productStore.uuid }
            });
    ...
    checkUuid = throttle(300, (rename?: boolean): void => {
        ...
        if (!this.editorStore.loadExisting || rename) {
            ...
            if (illegalCharsContained.length) {
                ...
                this.editorStore.warning = 'badChar';
                this.checkingUuid = false;
                return;
            }

            if (!rename && !this.productStore.uuid) {
                if (!this.productStore.loadExisting) {
                    this.editorStore.error = true;
            ...
            fetch(
                ...
                if (rename || !this.productStore.loadExisting) this.checkingUuid = false;
     ...
     * TODO: this should just be routing to the editor,
     */
